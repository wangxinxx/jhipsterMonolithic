...
import com.huibozhixin.jhimonolithic.service.BaseQuestionService;
...
public class BaseQuestionResource {
    ...
    /**
     * GET  /base-questions : get all the baseQuestions with answers.
     *
     * @param pageable the pagination information
     * @return the ResponseEntity with status 200 (OK) and the list of baseQuestions in body
     */
    @GetMapping("/base-questions-answers")
    @Timed
    public ResponseEntity<List<BaseQuestion>> getAllBaseQuestionsWithAnswers(Pageable pageable) {
        log.debug("REST request to get a page of BaseQuestions with answers");
        Page<BaseQuestion> page = baseQuestionService.findAllWithAnswers(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/base-questions-answers");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
